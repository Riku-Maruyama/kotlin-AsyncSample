...
class MainActivity : AppCompatActivity() {
    ...
    companion object{
        ...
        //天気APIにアクセスするためのAPIキー
        private const val APP_ID = "f59844a4787782ce5ee98b2c7caac4ec"
    ...
    private fun createList() : MutableList<MutableMap<String , String>> {
        ...
        city = mutableMapOf("name" to "兵庫" , "q" to "Kobe")
        list.add(city)
        ...
        city = mutableMapOf("name" to "石川" , "q" to "Kanazawa")
        list.add(city)
    ...
    private fun receiveWeatherInfo(urlFull: String) {
        ...
        val executeReceiver = Executors.newSingleThreadExecutor()
        executeReceiver.submit(backgroundReceiver)
    }
    ...
    private inner class WeatherInfoBackgroundReceiver(handler: Handler, url: String) : Runnable{
        ...
        override fun run(){
            ...
            //お天情報サービスから取得したJSON文字列。天気情報が格納されている。
            var result = ""
            //URLオブジェクトを生成
            val url = URL(_url)
    ...
    //非同期でお天気情報を取得したのちにUIスレッドでその情報を表示するためのクラス
    private inner class WeatherInfoPostExecutor(result: String): Runnable{
        ...
        override fun run() {
            ...
            //天気の詳細情報を表示する文字列を生成
            val desc = "現在は${weather}です。¥n緯度は${latitude}度で経度は${longitude}度です"
